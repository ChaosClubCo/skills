...
Please use fix_skills_unified.py instead, which consolidates all 3 fix passes
into a single script with shared I/O and consistent CLI flags.

    python fix_skills_unified.py --all                 # Run all 3 passes (default)
    python fix_skills_unified.py --all --pass 1        # Run only pass 1
    python fix_skills_unified.py --category ai-agents
    python fix_skills_unified.py --skill ai-agents/ceo
    python fix_skills_unified.py --dry-run --all --verbose

--- Original docstring ---
Multi-pass quality fixer for SKILL.md files.
...
Usage:
    python fix_skills.py --all                         # Run all 3 passes (default)
    python fix_skills.py --all --pass 1                # Run only pass 1
    python fix_skills.py --all --pass 2                # Run only pass 2
    python fix_skills.py --all --pass 3                # Run only pass 3
    python fix_skills.py --all --pass all              # Run all 3 passes
    python fix_skills.py --category ai-agents
    python fix_skills.py --skill ai-agents/ceo
    python fix_skills.py --dry-run --all
    python fix_skills.py --dry-run --all --verbose
"""
...
def fix_skill_file(
    ...
    When running all passes, the file is read once, all passes are applied
    in memory, and the file is written once.
    ...
    for pass_num in passes:
        if pass_num == 1:
            ...
            # Check for skip signals embedded in fixes
            ...
            # Filter out skip/error signals from fixes
            ...
            if skip_fixes and not real_fixes and len(passes) == 1:
                # Only skip if this is the sole pass being run
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-pass quality fixer for SKILL.md files",
    ...
    for skill_path in skills:
        ...
        try:
            ...
            if status in ("FIXED", "WOULD_FIX") or args.verbose:
                icon = {
                    "FIXED": "FIX",
                    "WOULD_FIX": "DRY",
                    "NO_CHANGE": " OK",
    ...
    print(f"  Skipped:   {stats.get('SKIP_EMPTY', 0) + stats.get('SKIP', 0)}")
