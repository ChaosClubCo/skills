...
version: "1.0.0"
description: >
  Deep analysis debugging mode for complex, stubborn, or mysterious issues. Activates methodical
  evidence-gathering, hypothesis testing, and rigorous verification protocol. Use when standard
  troubleshooting fails, when you're stuck on a bug, or when quick fixes keep breaking things.
  Triggers on: can't figure this out, spent hours on this bug, nothing is working, something is
  wrong but I don't know what, weird behavior, intermittent bug, flaky issue, root cause analysis,
  investigate this error, systematic debug, this makes no sense, bug keeps coming back, I fixed it
  but it broke again, production issue, mysterious failure, trace this error, debug like an expert,
  deep debug, or any request for methodical investigation rather than a quick patch. Also triggers
  when user has already tried obvious fixes and needs a structured approach to find the real cause.
---
